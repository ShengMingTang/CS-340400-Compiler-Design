...
| expr assignment expr %prec ASSIGNMENT {
    ...
    if(($1)[0] == '*'){ // @@ error prone
        int index = look_up_symbol($1+1);
        fprintf(f_asm, "  // expr= %s\n", $1);
        // *p = expr;
        // *p get evaluated first
        POP_STACK_t1(); // rhs
        POP_STACK_t0(); 
        fprintf(f_asm, "  // %s reference assignment\n", $1);
        
        fprintf(f_asm, "  addi t0, fp, %d\n", table[index].offset); // t0: address of expr(var)
        fprintf(f_asm, "  ld t0, 0(t0)\n"); // t0: address expr(var) is pointing to
...
| '-' expr %prec UPLUSMINUS {
    ...
    fprintf(f_asm, "  mul t0, t0, t1 // UMINUS\n");
    PUSH_STACK_t0();
    strcpy($$, $2);
}
| '&' expr %prec REF {
...
// evaluate *var (value it is pointing to)
// @@ problematic how can we tell it is on the LHS (get adress then reduce)
// or RHS (value should be retrived) 
| '*' expr %prec DEREF {
    int index = look_up_symbol($2);
    fprintf(f_asm, "  // evalute *%s\n", $2);
    POP_STACK_t0();
