...
// for labels
int labelStack[MAX_TABLE_SIZE];
int labelCount = 0;
int labelTop = 0;
int labelLastOffset[MAX_TABLE_SIZE];

void print_symbol_table()
{
    fprintf(f_asm, "// ====== symbol table ===== \n");
    for(int i = 0; i < cur_counter; i++){
        fprintf(f_asm, "// [%d] name:%s type:%d\n", i, table[i].name, table[i].type);
    }
    fprintf(f_asm, "// ====== symbol table ===== \n");
}
int push_label()
{
    labelStack[labelTop] = labelCount;
    labelCount++;
    labelTop++;
    return labelCount-1;
}
...
void stack_for_local_vars(char *var, char *cur_fn, int nElement)
{
    ...
    table[i].offset = -104 + table[fnIndex].offset - 8*nElement;
...
void stack_for_args(char* fn)
{
    ...
    if (index < 0){
        ...
        for(j = 1; j <= total_args; j++){
            ...
            table[index+j].offset = -104 + table[index].offset - 8*j;
...
void unstack_for_local_vars_and_args(char *fn)
{
    ...
    fprintf(f_asm, "  addi sp, sp, %d // unstack for locals(%d) and args(%d)\n", 
        ...
        -table[fnIndex].offset,
        table[fnIndex].total_locals,
        table[fnIndex].total_args
    );
}
void unstack_for_label(char *label, char *cur_fn)
{
    int blIndex = look_up_symbol(label);
    int fnIndex = look_up_symbol(label);
    if(blIndex < 0 || fnIndex < 0){
        perror("Cannot find function at func_def end");
        exit(1);
    }
    fprintf(f_asm, "  addi sp, sp, %d // unstack for label %s, locals(%d) and args(%d)\n", 
        -table[blIndex].offset,
        table[blIndex].name,
        table[blIndex].total_locals,
        table[blIndex].total_args
    );
    // at this point, cur_fn's offset has been enlarged
    int temp = table[fnIndex].offset;
    table[fnIndex].offset -= table[blIndex].offset;
    fprintf(f_asm, "  // cur_fn now stack is %s, before=%d, after=%d\n",
        table[fnIndex].name,
        temp,
        table[fnIndex].offset
    );
...
void enter_local_scope(char *fn)
{
    ...
    labelLastOffset[cur_scope] = table[index].offset;
    cur_scope++;
}
void leave_local_scope(char *fn)
{
    // restore sp as if the local scope never happens
    int index = look_up_symbol(fn);
    fprintf(f_asm, "  addi sp, sp, %d // leave local scope\n", 
        labelLastOffset[cur_scope-1] - table[index].offset
    );
    table[index].offset = labelLastOffset[cur_scope-1];
}
/*
...
int install_symbol(char *s)
{
    ...
    else
    {
        ...
        cur_counter++;
        fprintf(f_asm, "// symbol=%s installed, counter=%d, scope=%d\n", s, cur_counter, cur_scope);
    }
...
void pop_up_symbol(int scope)
{
    ...
    if (cur_counter == 0)
        return;
    for(i = 0; i < cur_counter; i++){
        fprintf(f_asm, "// Before func_def [%d] name:%s ,scope:%d popped\n", i, table[i].name, table[i].scope);
    }
