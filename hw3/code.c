...
void print_symbol_table()
{
    ...
    for(int i = 0; i < cur_counter; i++){
        fprintf(f_asm, "// [%d] name:%s type:%d, scope:%d\n", i, table[i].name, table[i].type, table[i].scope);
    }
    fprintf(f_asm, "// ====== symbol table ===== \n");
}
void print_fn(struct Func fn)
{
    fprintf(f_asm, "// ***** func info *****\n");
    fprintf(f_asm, "// name: %s, numArgs: %d\n", fn.name, fn.numArgs);
    for(int i = 0; i < fn.numArgs; i++){
        fprintf(f_asm, "// arg[%d]: %s\n", i, fn.args[i]);
    }
}
int push_label()
{
...
void leave_local_scope(char *fn)
{
    ...
    // fprintf(f_asm, "  addi sp, fp, %d // leave local scope\n", labelLastOffset[cur_scope-1]);
