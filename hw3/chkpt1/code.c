...
{
    fprintf(f_asm, "  addi sp, sp, -104 // BEGIN %s PROLOGUE\n", fn);
...
void stack_for_local_vars(char *var, char *cur_fn)
{
    ...
    fprintf(f_asm, "  addi sp, sp, -8 // local=%s parent=%s, offset=%d\n", var, table[fnIndex].name, table[i].offset);
}
